...
export default function HrmEmployees() {
  ...
  const admins = users.filter(u => u.hrm_role !== 'EMPLOYEE')
  const employees = users.filter(u => u.hrm_role === 'EMPLOYEE')

  const renderUser = (u: HrmUser) => {
    const roleColor = ROLE_COLORS[u.hrm_role] ?? colors.slate400
    const isEmployee = u.hrm_role === 'EMPLOYEE'
    return (
      <Card key={u.id} style={s.card}>
        <View style={s.row}>
          <View style={[s.avatar, isEmployee && { backgroundColor: colors.blue + '22', borderColor: colors.blue + '50' }]}>
            <Text style={[s.avatarText, isEmployee && { color: colors.blue }]}>{initials(u.full_name, u.email)}</Text>
          </View>
          <View style={s.info}>
    ...
    <ScrollView style={s.scroll} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.gold} />}>
      <View style={s.container}>
        <View style={s.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.pageTitle}>Team Members</Text>
            <Text style={s.pageSub}>{users.length} total HRM users</Text>
          </View>
        </View>
        ...
        <Text style={[s.sectionLabel, { marginTop: 8 }]}>EMPLOYEES ({employees.length})</Text>
        {employees.length === 0
          ? <Card><Text style={s.emptyText}>No employees yet.</Text></Card>
