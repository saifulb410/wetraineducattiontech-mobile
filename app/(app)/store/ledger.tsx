...
export default function Ledger() {
  ...
  const fetchData = async () => {
    ...
    let q = supabase
      ...
      .select('id,user_id,category,amount,reason,created_at')
    ...
    if (isAdmin && showAll) {
      ...
      if (ids.length > 0) {
        ...
        setEntries(((data ?? []) as AccountEntry[]).map(e => ({ ...e, profiles: profileMap[e.user_id] ?? null })))
      } else {
        setEntries([])
      }
    } else {
              ...
              <Card key={entry.id} style={s.entryCard}>
                <View style={s.entryRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {entry.reason ? (
                      <Text style={s.entryNotes} numberOfLines={1}>{entry.reason}</Text>
                    ) : null}
